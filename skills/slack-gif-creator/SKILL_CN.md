...
description: 用于创建 Slack 优化动画 GIF 的知识和工具。提供约束、验证工具和动画概念。当用户请求用于 Slack 的动画 GIF 时使用，如"给我做一个 X 做 Y 的 GIF 用于 Slack"。
...
- 层叠多个形状以增加复杂性（例如，一个星形内有一个更小的星形）

**使形状更有趣**：
- 不要只画一个普通的圆——添加高光、环或图案
- 星形可以有光晕（在后面绘制更大的、半透明的版本）
- 组合多个形状（星形 + 闪烁、圆形 + 环）
...
要有创意和细致！好的 Slack GIF 应该看起来精致，而不是像占位图形。
...
### 弹跳
对象下落并弹跳：
- 使用 `interpolate()` 配合 `easing='bounce_out'` 用于落地
...
- 起始位置：帧边界外
- 结束位置：目标位置
- 使用 `interpolate()` 配合 `easing='ease_out'` 进行平滑停止
...
**关于用户上传的注意**：此技能不包含预构建的图形，但如果用户上传图像，使用 PIL 加载和处理它——根据他们的请求解释是直接使用还是仅作为灵感。
