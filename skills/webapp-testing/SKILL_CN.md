...
**始终先使用 `--help` 运行脚本**以查看用法。在尝试运行脚本并发现绝对需要自定义解决方案之前，不要阅读源代码。这些脚本可能非常大，因此会污染你的上下文窗口。它们存在是为了作为黑盒脚本直接调用，而不是被摄入你的上下文窗口。
...
- **将捆绑脚本作为黑盒使用** - 要完成任务，考虑 `scripts/` 中的某个脚本是否能帮助。这些脚本可靠地处理常见、复杂的工作流程，而不会使上下文窗口混乱。使用 `--help` 查看用法，然后直接调用。
